...
function condat_demo_gpu
tic
	Nbiter= gpuArray(5000);	% number of iterations
	lambda1 = gpuArray(0.102021); 	% regularization parameter
	lambda2 = gpuArray(4.512587);	% regularization parameter
  
	tau = gpuArray(0.01);		% proximal parameter >0; influences the
                    % convergence speed
    ...
    %PSNR
    psnr_noisy = psnr(gather(noise_img),gather(GroTru));
    psnr_tgv = psnr(gather(denoise_img),gather(GroTru));

%SSIM
[mssim, ssim_map] = ssim(gather(denoise_img), gather(GroTru));
