...
for i=1:M           % paste all random parameters of noise and TGV to allPar
    ...
    % Save data as .txt   
    txtfile=sprintf('DATA/datDuan_%d_%d_%s.txt',M,N,datestamp)
    writetable(tabPar,txtfile,'Delimiter','tab');

%% Save data as matlab type     

    savefile=sprintf('DATA/duan_%d_%d_par_%s.mat',M,N,datestamp) % create file name
    save(savefile,'result'); % save struct result to file only store parameters
    
    saveimage=sprintf('DATA/duan_%d_%d_img_%s.mat',M,N,datestamp)
